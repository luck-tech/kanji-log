...
export const AccountDeleteModal: React.FC<AccountDeleteModalProps> = ({
	...
	const dataItems = [
		{ icon: 'calendar-outline', label: '作成したイベント', description: '日程調整、メンバー情報、Webフォーム' },
		{ icon: 'people-outline', label: 'メンバーリスト', description: '登録したメンバーの連絡先と情報' },
		{ icon: 'restaurant-outline', label: '開催記録', description: '過去のイベントログと評価' },
		{ icon: 'share-social-outline', label: '共有した記録', description: '他の幹事に公開した記録' },
		{ icon: 'settings-outline', label: 'アカウント設定', description: 'プロフィール情報と設定' },
	];
			...
			<SafeAreaView className="flex-1 bg-neutral-50">
				...
				<View className="px-6 py-4 bg-white border-b border-neutral-200">
					<View className="flex-row justify-between items-center">
						...
						<Text className="text-lg font-bold text-error-600">アカウント削除</Text>
						<View className="w-10" />
					</View>
				</View>
					...
					<View className="p-6 gap-6">
							...
							<View className="gap-4">
								...
								<View className="gap-3">
									{dataItems.map((item, index) => (
										<View key={index} className="flex-row items-start gap-3 p-3 bg-neutral-50 rounded-xl">
						...
						<Card variant="elevated" shadow="soft">
							<View className="gap-4">
								...
								<Text className="text-sm text-neutral-600 leading-5">
									...
									<Text className="font-bold text-error-600">「削除を確認」</Text>
									と入力してください。
								</Text>
								...
								<Input
									...
									error={confirmationText.length > 0 && !isConfirmed ? 'テキストが正しくありません' : undefined}
								/>
						...
						<Card variant="elevated" shadow="soft">
							<View className="gap-4">
								...
								<Text className="text-sm text-neutral-600 leading-5">
									アカウントを削除する前に、以下の選択肢もご検討ください：
								</Text>

								<View className="gap-2">
									<View className="flex-row items-center gap-3 p-3 bg-blue-50 rounded-xl">
										<Ionicons name="pause-circle" size={20} color="#0284c7" />
										<Text className="flex-1 text-blue-800 font-medium">
											一時的にアプリの使用を停止する
										</Text>
									</View>
									<View className="flex-row items-center gap-3 p-3 bg-blue-50 rounded-xl">
