...
import { DateScheduleModal, ScheduleData } from '@/components/modals/DateScheduleModal';
...
const getMockEvent = (id: string): Event => {
	const baseEvent = {
		...
		organizerId: '1',
		members: [
			{ id: '1', userId: '1', name: '田中太郎', email: 'tanaka@example.com', responseStatus: 'pending' as const, dateResponses: [], joinedAt: '2024-01-15' },
			{ id: '2', userId: '2', name: '佐藤花子', email: 'sato@example.com', responseStatus: 'pending' as const, dateResponses: [], joinedAt: '2024-01-15' },
			{ id: '3', userId: '3', name: '鈴木次郎', email: 'suzuki@example.com', responseStatus: 'pending' as const, dateResponses: [], joinedAt: '2024-01-15' },
		],
	...
	switch (id) {
		...
		case '2': // 確定済み
			return {
				...
				members: [
					{ id: '1', userId: '1', name: '田中太郎', email: 'tanaka@example.com', responseStatus: 'accepted' as const, dateResponses: [], joinedAt: '2024-01-15' },
					{ id: '2', userId: '2', name: '佐藤花子', email: 'sato@example.com', responseStatus: 'accepted' as const, dateResponses: [], joinedAt: '2024-01-15' },
					{ id: '3', userId: '3', name: '鈴木次郎', email: 'suzuki@example.com', responseStatus: 'accepted' as const, dateResponses: [], joinedAt: '2024-01-15' },
				],
				dateOptions: [
					{
						id: '1',
...
export default function EventDetailScreen() {
	...
	const handleAreaSelect = (areaType: 'center' | 'specified', area?: string) => {
	...
	const handleViewRestaurantSuggestions = () => {
		router.push(`/event/${id}/restaurant-suggestions`);
	};
	...
	const getResponseStats = () => {
		...
		const responded = event.members.filter(m => m.responseStatus !== 'pending').length;
					...
					<View className="p-6 gap-6">
						...
						<View className="gap-4">
							...
							{event.status !== 'completed' &&
								!(event.status === 'confirmed' && responseStats.responded === responseStats.total) && (
									<TouchableOpacity onPress={handleCreateForm} activeOpacity={0.8}>
							...
							{event.status === 'planning' && (
								<TouchableOpacity onPress={handleScheduleSetup} activeOpacity={0.8}>
									<Card variant="elevated" shadow="medium" animated={true}>
										<View className="flex-row items-center">
							...
							{/* 日程調整結果 */}
							{event.status === 'planning' && event.dateOptions && event.dateOptions.length > 0 && (
								<TouchableOpacity onPress={handleViewScheduleResults} activeOpacity={0.8}>
									<Card variant="elevated" shadow="medium" animated={true}>
										<View className="flex-row items-center">
							...
							{/* レストラン提案 - 確定済みのみで表示 */}
							{event.status === 'confirmed' && responseStats.responded === responseStats.total && (
								<TouchableOpacity onPress={handleAreaSelection} activeOpacity={0.8}>
									<Card variant="elevated" shadow="medium" animated={true}>
										<View className="flex-row items-center">
							...
							{/* イベント記録 - 開催済みのみで表示 */}
							{event.status === 'completed' && (
								<TouchableOpacity onPress={() => console.log('Open event log')} activeOpacity={0.8}>
									<Card variant="elevated" shadow="medium" animated={true}>
										<View className="flex-row items-center">
						...
						<Card variant="elevated" shadow="large" animated={true}>
							<View className="gap-4">
								<View className="flex-row items-center justify-between">
									<Text className="text-lg font-bold text-neutral-900">
										{event.status === 'completed' ? '参加者' : '参加メンバー'} ({event.members.length}名)
									</Text>
									{event.status !== 'completed' && (
								...
								<View className="gap-3">
									{event.members.map((member, index) => (
										...
										>
											...
											<View className={`px-2 py-1 rounded-full ${member.responseStatus === 'accepted' ? 'bg-success-100' :
												member.responseStatus === 'declined' ? 'bg-error-100' :
													'bg-neutral-100'
												}`}>
												<Text className={`text-xs font-medium ${member.responseStatus === 'accepted' ? 'text-success-700' :
													member.responseStatus === 'declined' ? 'text-error-700' :
														'text-neutral-600'
													}`}>
													{event.status === 'completed' ? '参加' :
														member.responseStatus === 'accepted' ? '参加' :
															member.responseStatus === 'declined' ? '不参加' :
																'未回答'}
												</Text>
											</View>
